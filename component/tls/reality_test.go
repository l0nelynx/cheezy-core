...
func TestRealityChromeClientHelloPreservesFingerprint(t *testing.T) {
	...
	if got, want := [3]byte(plainSessionID[:3]), [3]byte{26, 7, 28}; got != want {
...
func TestRealityVerifierRequiresConfiguredMldsa65Extension(t *testing.T) {
	...
	defer client.Close()
	defer server.Close()
	withMldsa := &realityVerifier{authKey: authKey, mldsa65Verify: make([]byte, 1952), UConn: utls.UClient(client, &utls.Config{}, utls.HelloChrome_Auto)}
	withMldsa.HandshakeState.Hello = &utls.PubClientHelloMsg{Raw: []byte("client hello")}
	withMldsa.HandshakeState.ServerHello = &utls.PubServerHelloMsg{Raw: []byte("server hello")}
